...
torch::Tensor Darknet::write_results(torch::Tensor prediction, int num_classes, float confidence, float nms_conf, float scale)
{
	...
	box_a.select(2, 0) = prediction.select(2, 0) - prediction.select(2, 2).div(2)*scale;
	box_a.select(2, 1) = prediction.select(2, 1) - prediction.select(2, 3).div(2)*scale;
	box_a.select(2, 2) = prediction.select(2, 0) + prediction.select(2, 2).div(2)*scale;
	box_a.select(2, 3) = prediction.select(2, 1) + prediction.select(2, 3).div(2)*scale;
